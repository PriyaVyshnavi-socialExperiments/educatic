...
export class SqliteStorageService {
     ...
     */
    async openStore(_table?: string): Promise<boolean> {
        if (this.isService) {
            const database: string = environment.sqlLiteDBName;
            const table: string = _table ? _table : 'storage_table';
            const encrypted = true;
            const { result } = await this.store.openStore({ database, table, encrypted });
            return result;
        } else {
